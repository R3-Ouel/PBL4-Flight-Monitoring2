...
async def download_excel():
    try:
        # Use an absolute path so the endpoint works regardless of CWD.
        base_dir = Path(__file__).resolve().parent
        csv_path = base_dir / "data" / "flight_data.csv"
        ...
        # The CSV can contain rows with a different number of columns than the
        # header. We load it defensively: for each row we truncate/pad it to
        # the header length so excel generation never fails on "tokenizing"
        # issues.
        ...
        for title, _cols, col_indices in charts:
        ...
        # Try to copy the generated file to the user's Downloads folder
